...
public partial class DizimistaDetalhesViewModel : ObservableObject, INavigationAware
{
    ...
    public DizimistaDetalhesViewModel(InativarDizimistaHandler inativarHandler,
        GetDizimistaHandlers getDizimistaHandlers, IUnitOfWork unitOfWork, IDialogService dialogService,
        INavigationService navigationService)
    {
        ...
        Debug.WriteLine("[INFO] DizimistaDetalhesViewModel construtor executado");
    }
    ...
    public Dizimista? Dizimista
    {
        ...
        set
        {
            Debug.WriteLine($"[INFO] DizimistaDetalhesViewModel.Dizimista sendo definido: {value?.Nome ?? "NULL"}");
    ...
    public string EnderecoCompleto
    {
        get
        {
            ...
            if (_dizimista == null)
            {
                Debug.WriteLine("[WARN] EnderecoCompleto: Dizimista � null");
                return string.Empty;
            }
            ...
            {
                Debug.WriteLine("[WARN] EnderecoCompleto: Endereco � null");
                return string.Empty;
            }
            ...
            {
                Debug.WriteLine("[WARN] EnderecoCompleto: Rua ou N�mero vazios");
                return string.Empty;
            }
            ...
            try
            {
                ...
                var enderecoStr =
                    $"{endereco.Rua}, {endereco.Numero}{complemento} - {endereco.Bairro}, {endereco.Cidade} - {endereco.UF}, {cepFormatado}";

                Debug.WriteLine($"[INFO] EnderecoCompleto: {enderecoStr}");
                return enderecoStr;
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[ERRO] Erro ao montar EnderecoCompleto: {ex.Message}");
                return string.Empty;
            }
        }
    }
    ...
    void INavigationAware.OnNavigatedTo(NavigationParameters parameters)
    {
        ...
        if (dizimistaId.HasValue && dizimistaId.Value != Guid.Empty) _ = LoadDizimistaAsync(dizimistaId.Value);
    }
    ...
    {
        Debug.WriteLine($"[INFO] LoadDizimistaAsync chamado com ID: {id}");
        _currentDizimistaId = id;
        try
        {
            var dizimista = await _getDizimistaHandlers.Handle(new GetDizimistaByIdQuery(id));
            Debug.WriteLine($"[INFO] Dizimista retornado do handler: {dizimista?.Nome ?? "NULL"}");

            if (dizimista != null)
            {
                Debug.WriteLine(
                    $"[INFO] Dizimista detalhes - Nome: {dizimista.Nome}, Telefone: {dizimista.Telefone}, Whatsapp: {dizimista.Whatsapp}");
                Debug.WriteLine($"[INFO] Endereco objeto � null? {dizimista.Endereco == null}");

                if (dizimista.Endereco != null)
                {
                    Debug.WriteLine($"[INFO] Endereco - Rua: {dizimista.Endereco.Rua}");
                    Debug.WriteLine($"[INFO] Endereco - N�mero: {dizimista.Endereco.Numero}");
                    Debug.WriteLine($"[INFO] Endereco - Complemento: {dizimista.Endereco.Complemento}");
                    Debug.WriteLine($"[INFO] Endereco - Bairro: {dizimista.Endereco.Bairro}");
                    Debug.WriteLine($"[INFO] Endereco - Cidade: {dizimista.Endereco.Cidade}");
                    Debug.WriteLine($"[INFO] Endereco - UF: {dizimista.Endereco.UF}");
                    Debug.WriteLine($"[INFO] Endereco - CEP: {dizimista.Endereco.CEP}");
                }
                else
                {
                    Debug.WriteLine("[ERRO] Endereco � NULL ap�s carregar o dizimista!");
                }

                Debug.WriteLine($"[INFO] Status Ativo: {dizimista.Ativo}");
            }
            else
            {
                Debug.WriteLine("[ERRO] Dizimista carregado � NULL!");
            }
        ...
        {
            Debug.WriteLine($"[ERRO] Erro ao carregar dizimista: {ex.Message}");
    ...
    private async Task CarregarUltimaOfertaAsync(Guid dizimistaId)
    {
        try
        {
            Debug.WriteLine($"[INFO] CarregarUltimaOfertaAsync chamado para ID: {dizimistaId}");
            ...
            if (ofertas != null)
            {
                ...
                if (ultimaOferta != null)
                {
                    ...
                    Debug.WriteLine($"[INFO] �ltima oferta encontrada: {UltimaOferta}");
                }
                else
                {
                    UltimaOferta = "Nenhuma oferta registrada";
                    Debug.WriteLine("[INFO] Nenhuma oferta encontrada");
                }
            }
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"[ERRO] Erro ao carregar �ltima oferta: {ex.Message}");
    ...
    public async Task EditarAsync()
    {
        if (_dizimista != null)
        {
            Debug.WriteLine($"[NAV] Editando dizimista: {_dizimista.Id}");
    ...
    public async Task ExcluirAsync()
    {
        if (_dizimista != null)
            try
            {
                ...
                Debug.WriteLine($"[INFO] ExcluirAsync - Excluindo dizimista ID: {_dizimista.Id}");

                await _unitOfWork.Dizimistas.DeleteAsync(_dizimista.Id);
                await _unitOfWork.SaveChangesAsync();

                Debug.WriteLine("[INFO] ExcluirAsync - Dizimista excluído com sucesso");
                await _dialogService.ShowSuccessAsync("Dizimista excluído com sucesso!");

                Debug.WriteLine("[NAV] Navegando para lista de dizimistas");
                _navigationService.Navigate("dizimistas");
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[ERRO] ExcluirAsync - Erro ao excluir: {ex.Message}");
                Debug.WriteLine($"[ERRO] Stack trace: {ex.StackTrace}");
    ...
    public void Voltar()
    {
        try
        {
            ...
            Debug.WriteLine("[NAV] Voltou da página de detalhes");
        }
        catch (Exception ex)
        {
