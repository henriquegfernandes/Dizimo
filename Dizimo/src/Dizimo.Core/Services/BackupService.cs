...
namespace Dizimo.Core.Services;
        ...
        if (root.TryGetProperty("Ofertas", out var ofertasArr))
        {
            foreach (var el in ofertasArr.EnumerateArray())
            {
                try
                {
                    ...
                    var dataStr = el.GetProperty("Data").GetString();
                    if (string.IsNullOrWhiteSpace(dataStr)) continue;
                    if (!DateTime.TryParse(dataStr, out var data)) continue;
                    var obs = el.TryGetProperty("Observacao", out var o) ? o.GetString() ?? string.Empty : string.Empty;

                    // Map original dizimista id -> new id if available. Do NOT reuse original numeric ID
                    // unless it was remapped; that avoids creating ofertas referring to non-existent dizimistas.
                    var targetId = 0;
                    if (dizId != 0 && idMap.TryGetValue(dizId, out var mapped))
                    {
                        targetId = mapped;
                    }
